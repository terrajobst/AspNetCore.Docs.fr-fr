...
| [Cookie.Expiration](/dotnet/api/microsoft.aspnetcore.http.cookiebuilder.expiration?view=aspnetcore-2.0) | Obtient ou définit la durée de vie d’un cookie. Actuellement, cette option ne fait rien et deviendront obsolète dans ASP.NET Core 2.1 +. Utilisez la `ExpireTimeSpan` option permettant de définir l’expiration du cookie. Pour plus d’informations, consultez [clarifier le comportement de CookieAuthenticationOptions.Cookie.Expiration](https://github.com/aspnet/Security/issues/1293). |
...
Vous pouvez définir une heure d’expiration absolue avec `ExpiresUtc`. Vous devez également définir `IsPersistent`; sinon, `ExpiresUtc` est ignoré et un cookie de session unique est créé. Lorsque `ExpiresUtc` est définie sur `SignInAsync`, il remplace la valeur de la `ExpireTimeSpan` option de `CookieAuthenticationOptions`, si définie.
