...
* L’application Blazor est placée sur un service ou un serveur web d’hébergement statique, où .NET n’est pas utilisé pour servir l’application Blazor. Cette stratégie est traitée dans la section [Déploiement autonome](#standalone-deployment) , qui comprend des informations sur l’hébergement d’une application éblouissante côté client en tant que sous-application IIS.

## <a name="rewrite-urls-for-correct-routing"></a>Réécriture d’URL pour un routage correct

Les requêtes de routage pour les composants de page dans une application côté client ne sont pas aussi simples que les demandes de routage dans une application hébergée côté serveur. Considérez une application côté client avec deux composants :
