...
---
# <a name="create-read-update-and-delete---ef-core-with-razor-pages-2-of-8"></a>Créer, lire, mettre à jour et supprimer - EF Core avec les Pages Razor (2 sur 8)
...
Dans ce didacticiel, le code des méthodes CRUD généré automatiquement (créer, lire, mettre à jour, supprimer) est révisé et personnalisé.

Remarque : Pour réduire la complexité et conserver ces didacticiels axés sur EF Core, le code EF Core est utilisé dans les modèles de page des Pages Razor. Certains développeurs utilisent une couche de service ou un pattern repository dans pour créer une couche d’abstraction entre l’interface utilisateur (Pages Razor) et la couche d’accès aux données.

Dans ce didacticiel, les pages Razor pour créer, modifier, supprimer et afficher détails dans le dossier *student* sont modifiées.

Le code généré automatiquement utilise le modèle suivant pour créer, modifier et supprimer des pages :
...
Les pages Index et Details obtenennenti et affichent les données demandées avec la méthode HTTP GET`OnGetAsync`

## <a name="replace-singleordefaultasync-with-firstordefaultasync"></a>Remplacez SingleOrDefaultAsync avec FirstOrDefaultAsync
...
* `SingleOrDefaultAsync` récupère plus de données et effectue le travail non nécessaire.
...
* Peut avoir des avantages de performances dans certaines situations, mais ils interviennet rarement pour les scénarios web normaux.
...
Toutefois, si vous souhaitez inclure d’autres entités, le Find n’est plus approprié. Cela signifie que vous devrez peut-être abandonner Find et passer à une requête fur et à mesure que votre application progresse.

## <a name="customize-the-details-page"></a>Personnaliser la page de détails

Accédez à la page `Pages/Students`. Le liens **Edit**, **Details**, et **Delete** sont générés par le [Tag helper anchor](xref:mvc/views/tag-helpers/builtin-th/anchor-tag-helper) dans le fichier *Pages/Student/Index.cshtml*.
...
Mettez à jour l’édition, les Pages Razor Details et Delete pour utiliser le modèle de routing `"{id:int}"`. Remplacez la directive de chacune de ces pages (`@page "{id:int}"`) par `@page`.

Une demande à la page avec le modèle de routing «{id:int}» qui n'inclut **pas** valeur de routing avec un entier renvoie une erreur HTTP 404 (not found). Par exemple, `http://localhost:5000/Students/Details` renvoie une erreur 404. Pour que l’ID soit facultatif, ajoutez `?` à la contrainte de routing :

 ```cshtml
@page "{id:int?}"
```

Exécuter l’application, cliquez sur un lien de détails et vérifier l’URL est en passant l’ID en tant que données de routing (`http://localhost:5000/Students/Details/2`).

Ne modifiez pas globalement `@page` à `@page "{id:int}"`, cela casserait les liens vers la page d’accueil et les pages de création.

<!-- See https://github.com/aspnet/Scaffolding/issues/590 -->

### <a name="add-related-data"></a>Ajouter des données associées

Le code généré automatiquement pour la page d’Index des étudiants n’inclut pas la propriété `Enrollments`. Dans cette section, le contenu de la collection `Enrollments` s’affiche dans la page de détails.
...
Les méthodes `Include` et `ThenInclude` font en sorte que le contexte chargent le propriété de navigation `Student.Enrollments` et dans chaque inscription la propriété de navigation `Enrollment.Course`. Ces méthodes sont examinées en détail dans le didacticiel de la lecture des données.
...
### <a name="display-related-enrollments-on-the-details-page"></a>Afficher les inscriptions associées sur la page de détails
...
Exécuter l’application, sélectionnez l'onglet **Students**, puis cliquez sur le lien **détails** pour un étudiant. La liste des cours et les notes de l’étudiant sélectionnés s’affiche.
...
* Le deuxième argument (` "student", // Prefix`) est le préfixe utilisé pour rechercher des valeurs. Il nz respecte pas la casse.
* Les valeurs de formulaire publiées sont converties en types dans le modèle `Student` en utilisant la [liaison de modèle](xref:mvc/models/model-binding#how-model-binding-works).

<a id="overpost"></a>
### <a name="overposting"></a>Overposting

Utiliser `TryUpdateModel` pour mettre à jour les champs avec les valeurs publiées est une meilleure pratique de sécurité, car elle empêche l'overposting. Par exemple, supposons que l’entité Student inclut une propriété `Secret` que cette page web ne doive pas mettre à jour ou ajouter :

[!code-csharp[Main](intro/samples/cu/Models/StudentZsecret.cs?name=snippet_Intro&highlight=7)]

Même si l’application n’a pas un champ `Secret` sur la création/mise à jour Page Razor, un pirate peut définir la valeur `Secret` en la repostant. Un pirate peut utiliser un outil tel que Fiddler ou écrire du JavaScript, pour valider une valeur de formulaire `Secret`. Le code d’origine ne limite pas les champs que le classeur de modèles utilise lorsqu’il crée une instance de l’étudiant.

Laleur que le pirate spécifie pour le `Secret` champ de formulaire est mise à jour dans la base de données. L’illustration suivante montre l’outil Fiddler ajoutant le champ `Secret` (avec la valeur « OverPost ») pour les valeurs de formulaire publiées.
...
Afficher les modèles fournissent un autre moyen d’empêcher l'overposting. Le modèle d’affichage contient uniquement les propriétés pour afficher ou mettre à jour.
...
* `Modified`: Tout ou un partie des valeurs de propriété de l’entité ont été modifiés. La méthode `SaveChanges` émet une instruction UPDATE.
...
Dans une application web, le `DbContext` qui lit une entité et affiche les données sont supprimées après le rendu d’une page. Lorsque la méthode `OnPostAsync` d'une page est appelée, une nouvelle requête web est effectuée avec une nouvelle instance de `DbContext`. La relecture de l’entité dans ce nouveau contexte simule le traitement de bureau.
...
* La méthode Delete des pages `OnGetAsync` est appelée avec `saveChangesError=true`.

### <a name="update-the-delete-razor-page"></a>Mise à jour de la Page Razor Delete

Ajouter le message d’erreur mis en surbrillance suivant à la Page Delete de Razor.
...
Vérifiez que la Page Razor contient la bonne directive `@page`. Par exemple, la Page Razor directive ne doit **pas** contenir un modèle de routing :
