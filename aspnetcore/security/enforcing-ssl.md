...
>
> Les projets d’API par défaut n’incluent pas [HSTS](#hsts) HSTS étant généralement une seule instruction de navigateur. Autres appelants, telles que le téléphone ou les applications de bureau, procédez comme **pas** respectent l’instruction. Même dans les navigateurs, un seul appel authentifié à une API via HTTP présente des risques sur les réseaux non sécurisés. L’approche sûre consiste à configurer des projets d’API pour écouter uniquement et y répondre via le protocole HTTPS.
...
L’installation du kit SDK .NET Core installe le certificat de développement ASP.NET Core HTTPS dans le magasin de certificats de l’utilisateur local. Le certificat a été installé, mais il n’a pas été approuvé. Pour approuver le certificat procédez à usage unique pour exécuter le dotnet `dev-certs` outil :
...
## <a name="trust-https-certificate-from-windows-subsystem-for-linux"></a>Faire confiance au certificat HTTPS à partir du sous-système de Windows pour Linux

Le sous-système Windows pour Linux (WSL) génère un certificat auto-signé HTTPS. Pour configurer le magasin de certificats Windows pour approuver le certificat WSL :

* Exécutez la commande suivante pour exporter le certificat WSL généré : `dotnet dev-certs https -ep %USERPROFILE%\.aspnet\https\aspnetapp.pfx -p <cryptic-password>`
* Dans une fenêtre WSL, exécutez la commande suivante : `ASPNETCORE_Kestrel__Certificates__Default__Password="<cryptic-password>" ASPNETCORE_Kestrel__Certificates__Default__Path=/mnt/c/Users/user-name/.aspnet/https/aspnetapp.pfx dotnet watch run`
  ...
  La commande précédente définit les variables d’environnement Linux utilise le certificat approuvé Windows.
