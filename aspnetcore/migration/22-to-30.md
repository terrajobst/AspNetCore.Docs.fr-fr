...
La modification la plus importante à partir de `WebHostBuilder` à `HostBuilder` est dans [l’injection de dépendance (DI)](xref:fundamentals/dependency-injection). Lorsque vous utilisez `HostBuilder`, vous pouvez injecter uniquement <xref:Microsoft.Extensions.Configuration.IConfiguration>, <xref:Microsoft.Extensions.Hosting.IHostingEnvironment>, et <xref:Microsoft.AspNetCore.Hosting.IHostingEnvironment> dans le constructeur du démarrage. Le `HostBuilder` les contraintes de l’injection de dépendances :
