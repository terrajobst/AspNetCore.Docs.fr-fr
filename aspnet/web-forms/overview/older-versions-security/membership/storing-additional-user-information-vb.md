...
- <strong>Créer un ou plusieurs</strong><strong>`WizardStep`</strong><strong>s pour contenir les éléments d’interface utilisateur</strong>. Pour ajouter un nouveau `WizardStep` CreateUserWizard, cliquez sur le « Ajout/suppression `WizardStep` s « lien à partir de sa balise active pour lancer le `WizardStep` éditeur de collections. À partir de là, vous pouvez ajouter, supprimer ou réorganiser les étapes dans l’Assistant. Il s’agit de l’approche que nous allons utiliser pour ce didacticiel.
