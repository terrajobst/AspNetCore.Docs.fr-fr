...
Le modèle d’hébergement intra-processus n’est pas pris en charge pour les applications ASP.NET Core qui ciblent le .NET Framework.
...
Si l’application appelle <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> dans le `CreateDefaultBuilder` méthode de la `Program` classe, appelez `ConfigureKestrel` pour configurer le serveur Kestrel au lieu de `UseKestrel` afin d’éviter les conflits avec la [IIS dans le processus modèle d’hébergement](xref:fundamentals/servers/index#in-process-hosting-model):
...
Si l’application n’appelle pas `CreateDefaultBuilder`, appelez <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **avant** appelant `ConfigureKestrel`:
...
* Déplacement de l’initialisation du fournisseur pour la journalisation `Startup.ConfigureServices`.
...
* Modification de filtrage pour utiliser le <xref:Microsoft.Extensions.DependencyInjection.LoggingServiceCollectionExtensions.AddLogging*> générateur.
...
* Modifier le chargement de configuration à utiliser le `AddLogging` générateur.
