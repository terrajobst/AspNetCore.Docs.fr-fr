...
* Traitez toute méthode .NET exposée à JavaScript comme vous le feriez pour un point de terminaison public de l’application.
  ...
  * Prenez en compte la possibilité d’exposer des méthodes statiques et d’instance à des clients JavaScript. Évitez de partager l’état entre les sessions, sauf si la conception appelle l’état de partage avec les contraintes appropriées.
    * Pour les méthodes d’instance `DotNetReference` exposées par le biais d’objets créés à l’origine par le biais de l’injection de dépendances, les objets doivent être inscrits en tant qu’objets délimités. Cela s’applique à tout service d’injection de services utilisé par l’application de serveur éblouissant.
...
Les recommandations relatives à la sécurisation des applications ASP.NET Core s’appliquent aux applications serveur éblouissantes et sont traitées dans les sections suivantes :
...
Le serveur éblouissant utilise Signalr pour la communication entre le client et le serveur. Le serveur éblouissant utilise normalement le transport négocié par Signalr, qui est généralement WebSocket.

Le serveur éblouissant ne garantit pas l’intégrité et la confidentialité des données envoyées entre le serveur et le client. Utilisez toujours le protocole HTTPs.
...
L’infrastructure de serveur éblouissant prend des mesures pour se protéger contre certaines des menaces précédentes :
...
Lors du démarrage d’une session d’application de serveur éblouissant, le serveur effectue une validation de base des URL envoyées dans le cadre du démarrage de la session. L’infrastructure vérifie que l’URL de base est un parent de l’URL actuelle avant d’établir le circuit. Aucune vérification supplémentaire n’est effectuée par l’infrastructure.
