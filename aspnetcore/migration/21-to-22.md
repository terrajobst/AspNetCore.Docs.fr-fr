...
Pour adopter le [modèle d’hébergement intra-processus pour IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), ajouter le `<AspNetCoreHostingModel>` propriété avec une valeur de `InProcess` à un `<PropertyGroup>` dans le fichier projet :
...
## <a name="update-a-custom-webconfig-file"></a>Mettre à jour un fichier web.config personnalisée

Pour les projets qui utilisent un personnalisé *web.config* fichier dans la racine du projet pour générer leurs publié *web.config* fichier :

* Dans le `<handlers>` entrée qui ajoute le Module ASP.NET Core (`name="aspNetCore"`), modifiez le `modules` valeur à partir de l’attribut `AspNetCoreModule` à `AspNetCoreModuleV2`.
* Dans le `<aspNetCore>` élément, ajoutez l’attribut de modèle hébergement (`hostingModel="InProcess"`).

Pour plus d’informations et exemple *web.config* de fichiers, consultez <xref:host-and-deploy/aspnet-core-module#configuration-with-webconfig>.
