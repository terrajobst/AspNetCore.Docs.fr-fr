...
> [!NOTE]
> Vous pouvez également fournir votre propre implémentation <xref:Microsoft.Azure.KeyVault.KeyVaultClient> à <xref:Microsoft.Extensions.Configuration.AzureKeyVaultConfigurationExtensions.AddAzureKeyVault*>. Un client personnalisé permet de partager une seule instance du client dans l’application.
...
Les secrets sont mis en cache jusqu'à ce que la méthode `IConfigurationRoot.Reload()` soit appelée. Les secrets expirés, désactivés et mis à jour dans le coffre de clés ne sont pas respectés par l’application tant que `Reload` n’est pas exécutée.
...
Les secrets désactivés et expirés lèvent une exception <xref:Microsoft.Azure.KeyVault.Models.KeyVaultErrorException>. Pour empêcher l’application de se déclencher, fournissez la configuration à l’aide d’un autre fournisseur de configuration ou mettez à jour le secret désactivé ou expiré.
