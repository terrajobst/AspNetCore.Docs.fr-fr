...
* Projets par défaut pour le [modèle d’hébergement intra-processus](xref:host-and-deploy/aspnet-core-module#in-process-hosting-model) dans ASP.NET Core 3.0 ou version ultérieure. Vous pouvez éventuellement supprimer le `<AspNetCoreHostingModel>` élément si sa valeur est `InProcess`.
