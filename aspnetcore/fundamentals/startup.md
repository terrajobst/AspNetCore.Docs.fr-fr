...
Les applications ASP.NET Core utilisent une classe `Startup`, qui est nommée `Startup` par convention. La classe `Startup` :
...
Les services supplémentaires, tels que `IHostingEnvironment` et `ILoggerFactory`, peuvent également être spécifiés dans la signature de méthode. Si spécifiés, les services supplémentaires sont injectés s'ils sont disponibles. 
...
* Plusieurs implémentations `IStartupFilter` peuvent interagir avec les mêmes objets. Si l’ordre est important, classez leurs inscriptions de service `IStartupFilter` pour les faire correspondre avec l’ordre dans lequel les middlewares doivent s'exécuter. 
