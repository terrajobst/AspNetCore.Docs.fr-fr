...
# <a name="aspnet-core-1xtabaspnetcore1x"></a>[<span data-ttu-id="467df-112">ASP.NET Core 1.x</span><span class="sxs-lookup"><span data-stu-id="467df-112">ASP.NET Core 1.x</span></span>](#tab/aspnetcore1x)
...
## <a name="how-to-add-providers"></a><span data-ttu-id="467df-123">Comment ajouter des fournisseurs</span><span class="sxs-lookup"><span data-stu-id="467df-123">How to add providers</span></span>
...
<span data-ttu-id="467df-148">Les autres sections de cet article détaillent certains points et présentent les options de journalisation.</span><span class="sxs-lookup"><span data-stu-id="467df-148">The remainder of this article explains some details and options for logging.</span></span>
...
## <a name="log-level"></a><span data-ttu-id="467df-160">Niveau de journalisation</span><span class="sxs-lookup"><span data-stu-id="467df-160">Log level</span></span>
...
* <span data-ttu-id="467df-190">Error = 4</span><span class="sxs-lookup"><span data-stu-id="467df-190">Error = 4</span></span>
...
## <a name="log-filtering"></a><span data-ttu-id="467df-228">Filtrage de journal</span><span class="sxs-lookup"><span data-stu-id="467df-228">Log filtering</span></span>
...
<span data-ttu-id="467df-240">**Règles de filtre dans le code**</span><span class="sxs-lookup"><span data-stu-id="467df-240">**Filter rules in code**</span></span>
...
| <span data-ttu-id="467df-275">7</span><span class="sxs-lookup"><span data-stu-id="467df-275">7</span></span>      | <span data-ttu-id="467df-276">Tous les fournisseurs</span><span class="sxs-lookup"><span data-stu-id="467df-276">All providers</span></span> | <span data-ttu-id="467df-277">Système</span><span class="sxs-lookup"><span data-stu-id="467df-277">System</span></span>                                  | <span data-ttu-id="467df-278">Déboguer</span><span class="sxs-lookup"><span data-stu-id="467df-278">Debug</span></span>             |
...
<span data-ttu-id="467df-327">La méthode d’extension `WithFilter` est fournie par le package NuGet [Microsoft.Extensions.Logging.Filter](https://www.nuget.org/packages/Microsoft.Extensions.Logging.Filter).</span><span class="sxs-lookup"><span data-stu-id="467df-327">The `WithFilter` extension method is provided by the [Microsoft.Extensions.Logging.Filter](https://www.nuget.org/packages/Microsoft.Extensions.Logging.Filter) NuGet package.</span></span> <span data-ttu-id="467df-328">La méthode retourne une nouvelle instance `ILoggerFactory` qui filtre les messages de journal passés à tous les fournisseurs de journalisation inscrits dans cette méthode.</span><span class="sxs-lookup"><span data-stu-id="467df-328">The method returns a new `ILoggerFactory` instance that will filter the log messages passed to all logger providers registered with it.</span></span> <span data-ttu-id="467df-329">Cela n’affecte pas les autres instances `ILoggerFactory`, y compris l’instance `ILoggerFactory` initiale.</span><span class="sxs-lookup"><span data-stu-id="467df-329">It does not affect any other `ILoggerFactory` instances, including the original `ILoggerFactory` instance.</span></span>
...
# <a name="aspnet-core-1xtabaspnetcore1x"></a>[<span data-ttu-id="467df-340">ASP.NET Core 1.x</span><span class="sxs-lookup"><span data-stu-id="467df-340">ASP.NET Core 1.x</span></span>](#tab/aspnetcore1x)
...
# <a name="aspnet-core-2xtabaspnetcore2x"></a>[<span data-ttu-id="467df-403">ASP.NET Core 2.x</span><span class="sxs-lookup"><span data-stu-id="467df-403">ASP.NET Core 2.x</span></span>](#tab/aspnetcore2x)
...
<span data-ttu-id="467df-416">Le fournisseur est activé uniquement si votre projet est exécuté dans l’environnement Azure.</span><span class="sxs-lookup"><span data-stu-id="467df-416">The provider only works when your project runs in the Azure environment.</span></span> <span data-ttu-id="467df-417">Il n’a aucun effet si vous l’exécutez localement &mdash; il n’écrit pas d’enregistrements dans les fichiers locaux ou dans le stockage de développement local pour les objets blob.</span><span class="sxs-lookup"><span data-stu-id="467df-417">It has no effect when you run locally &mdash; it does not write to local files or local development storage for blobs.</span></span>

## <a name="third-party-logging-providers"></a><span data-ttu-id="467df-418">Fournisseurs de journalisation tiers</span><span class="sxs-lookup"><span data-stu-id="467df-418">Third-party logging providers</span></span>

<span data-ttu-id="467df-419">Voici des frameworks de journalisation tiers qui sont pris en charge dans ASP.NET Core :</span><span class="sxs-lookup"><span data-stu-id="467df-419">Here are some third-party logging frameworks that work with ASP.NET Core:</span></span>

* <span data-ttu-id="467df-420">[elmah.io](https://github.com/elmahio/Elmah.Io.Extensions.Logging) : fournisseur pour le service Elmah.Io</span><span class="sxs-lookup"><span data-stu-id="467df-420">[elmah.io](https://github.com/elmahio/Elmah.Io.Extensions.Logging) - provider for the Elmah.Io service</span></span>

* <span data-ttu-id="467df-421">[JSNLog](http://jsnlog.com) : enregistre les exceptions JavaScript et d’autres événements côté client dans votre journal côté serveur.</span><span class="sxs-lookup"><span data-stu-id="467df-421">[JSNLog](http://jsnlog.com) - logs JavaScript exceptions and other client-side events in your server-side log.</span></span>

* <span data-ttu-id="467df-422">[Loggr](https://github.com/imobile3/Loggr.Extensions.Logging) : fournisseur pour le service Loggr</span><span class="sxs-lookup"><span data-stu-id="467df-422">[Loggr](https://github.com/imobile3/Loggr.Extensions.Logging) - provider for the Loggr service</span></span>

* <span data-ttu-id="467df-423">[NLog](https://github.com/NLog/NLog.Extensions.Logging) : fournisseur pour la bibliothèque NLog</span><span class="sxs-lookup"><span data-stu-id="467df-423">[NLog](https://github.com/NLog/NLog.Extensions.Logging) - provider for the NLog library</span></span>

* <span data-ttu-id="467df-424">[Serilog](https://github.com/serilog/serilog-extensions-logging) : fournisseur pour la bibliothèque Serilog</span><span class="sxs-lookup"><span data-stu-id="467df-424">[Serilog](https://github.com/serilog/serilog-extensions-logging) - provider for the Serilog library</span></span>

<span data-ttu-id="467df-425">Certains frameworks tiers prennent en charge la [journalisation sémantique, également appelée journalisation structurée](https://softwareengineering.stackexchange.com/questions/312197/benefits-of-structured-logging-vs-basic-logging).</span><span class="sxs-lookup"><span data-stu-id="467df-425">Some third-party frameworks can do [semantic logging, also known as structured logging](https://softwareengineering.stackexchange.com/questions/312197/benefits-of-structured-logging-vs-basic-logging).</span></span>

<span data-ttu-id="467df-426">L’utilisation d’un framework tiers est semblable à celle des fournisseurs intégrés : vous ajoutez un package NuGet à votre projet, puis vous appelez une méthode d’extension sur `ILoggerFactory`.</span><span class="sxs-lookup"><span data-stu-id="467df-426">Using a third-party framework is similar to using one of the built-in providers: add a NuGet package to your project and call an extension method on `ILoggerFactory`.</span></span> <span data-ttu-id="467df-427">Pour plus d’informations, consultez la documentation du framework.</span><span class="sxs-lookup"><span data-stu-id="467df-427">For more information, see each framework's documentation.</span></span>

<span data-ttu-id="467df-428">Vous pouvez également créer vos propres fournisseurs personnalisés pour prendre en charge d’autres frameworks de journalisation ou répondre à des besoins de journalisation particuliers.</span><span class="sxs-lookup"><span data-stu-id="467df-428">You can create your own custom providers as well, to support other logging frameworks or your own logging requirements.</span></span>

## <a name="azure-log-streaming"></a><span data-ttu-id="467df-429">Streaming des journaux Azure</span><span class="sxs-lookup"><span data-stu-id="467df-429">Azure log streaming</span></span>

<span data-ttu-id="467df-430">Le streaming des journaux Azure vous permet d’afficher l’activité de journalisation en temps réel à partir des emplacements suivants :</span><span class="sxs-lookup"><span data-stu-id="467df-430">Azure log streaming enables you to view log activity in real time from:</span></span> 

* <span data-ttu-id="467df-431">Serveur d’applications</span><span class="sxs-lookup"><span data-stu-id="467df-431">The application server</span></span> 
* <span data-ttu-id="467df-432">Serveur web</span><span class="sxs-lookup"><span data-stu-id="467df-432">The web server</span></span>
* <span data-ttu-id="467df-433">Suivi des demandes ayant échoué</span><span class="sxs-lookup"><span data-stu-id="467df-433">Failed request tracing</span></span> 

<span data-ttu-id="467df-434">Pour configurer le streaming des journaux Azure :</span><span class="sxs-lookup"><span data-stu-id="467df-434">To configure Azure log streaming:</span></span> 

* <span data-ttu-id="467df-435">Accédez à la page **Journaux de diagnostic** à partir du portail de votre application.</span><span class="sxs-lookup"><span data-stu-id="467df-435">Navigate to the **Diagnostics Logs** page from your application's portal page</span></span>
* <span data-ttu-id="467df-436">Définissez l’option **Journal des applications (Système de fichiers)** sur Activé.</span><span class="sxs-lookup"><span data-stu-id="467df-436">Set **Application Logging (Filesystem)** to on.</span></span> 

![Page Journaux de diagnostic du portail Azure](index/_static/azure-diagnostic-logs.png)

<span data-ttu-id="467df-438">Accédez à la page **Streaming des journaux** pour afficher les messages d’application.</span><span class="sxs-lookup"><span data-stu-id="467df-438">Navigate to the **Log Streaming** page to view application messages.</span></span> <span data-ttu-id="467df-439">Les messages sont enregistrés par application dans l’interface `ILogger`.</span><span class="sxs-lookup"><span data-stu-id="467df-439">They are logged by application through the `ILogger` interface.</span></span> 

![Streaming des journaux d’application dans le portail Azure](index/_static/azure-log-streaming.png)


## <a name="see-also"></a><span data-ttu-id="467df-441">Voir aussi</span><span class="sxs-lookup"><span data-stu-id="467df-441">See also</span></span>

[<span data-ttu-id="467df-442">Journalisation avancée avec LoggerMessage</span><span class="sxs-lookup"><span data-stu-id="467df-442">High-performance logging with LoggerMessage</span></span>](xref:fundamentals/logging/loggermessage)
