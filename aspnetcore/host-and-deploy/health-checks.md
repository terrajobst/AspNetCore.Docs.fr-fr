...
Les contrôles d’intégrité sont créés via l’implémentation de l’interface `IHealthCheck`. La méthode `IHealthCheck.CheckHealthAsync` retourne un `Task<HealthCheckResult>` qui indique l’état d’intégrité comme étant `Healthy`, `Degraded` ou `Unhealthy`. Le résultat est écrit sous la forme de texte en clair avec un code d’état configurable (la configuration est décrite dans la section [Options de contrôle d’intégrité](#health-check-options)). `HealthCheckResult` peut également retourner des paires clé-valeur facultatives.
...
Le type `ExampleHealthCheck` est ajouté aux services de contrôle d’intégrité avec `AddCheck` :
...
Dans `Startup.Configure`, appelez `UseHealthChecks` dans le pipeline de traitement avec l’URL de point de terminaison ou le chemin relatif :
...
Si les contrôles d’intégrité doivent écouter un port en particulier, utilisez une surcharge de `UseHealthChecks` pour définir le port (cette procédure est décrite plus loin dans la section [Filtrer par port](#filter-by-port)) :
...
Utilisez `ResultStatusCodes` pour personnaliser le mappage de l’état d’intégrité aux codes d’état HTTP. Les affectations `StatusCode` suivantes sont les valeurs par défaut utilisées par le middleware. Vous pouvez modifier les valeurs de code d’état selon vos besoins.
...
L’option `ResponseWriter` obtient ou définit un délégué utilisé pour écrire la réponse. Le délégué par défaut écrit une réponse minimale constituée de texte en clair, avec la valeur de chaîne `HealthReport.Status`.
...
Pour inscrire les services de contrôle d’intégrité, utilisez `AddHealthChecks` dans `Startup.ConfigureServices`. L’exemple d’application appelle la méthode `AddSqlServer` de BeatPulse avec la chaîne de connexion de la base de données (*DbHealthStartup.cs*) :
...
Si vous appelez `UseHealthChecks` avec un port, les requêtes de contrôle d’intégrité seront limitées au port spécifié. Ceci est généralement utilisé dans les environnements de conteneurs pour exposer un port aux services de supervision.
...
Pour inscrire les services de contrôle d’intégrité, utilisez `AddHealthChecks` dans `Startup.ConfigureServices`. L’appel à `UseHealthChecks` spécifie le port de gestion (*ManagementPortStartup.cs*) :

[!code-csharp[](health-checks/samples/2.x/HealthChecksSample/ManagementPortStartup.cs?name=snippet1&highlight=12,18)]

> [!NOTE]
> Vous pouvez éviter de créer le fichier *launchSettings.json* dans l’exemple d’application en définissant explicitement les URL et le port de gestion dans le code. Dans *Program.cs* où `WebHostBuilder` est créé, ajoutez un appel à `UseUrls` et fournissez le point de terminaison de réponse normal et le point de terminaison de port de gestion de l’application. Dans *ManagementPortStartup.cs* où `UseHealthChecks` est appelé, spécifiez le port de gestion explicitement.
>
> *Program.cs* :
>
> ```csharp
...
1. Écrivez une méthode d’extension avec des paramètres que l’application consommatrice appelle dans sa méthode `Startup.Configure`. Dans l’exemple qui suit, prenons la signature de méthode de contrôle d’intégrité suivante :
   ...
   * État d’échec (`HealthStatus`). La valeur par défaut est `null`. Si `null`, `HealthStatus.Unhealthy` est signalé pour un état d’échec.
