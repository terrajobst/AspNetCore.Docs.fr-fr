...
* Projets faisant référence à la `Microsoft.AspNetCore.All` ou `Microsoft.AspNetCore.App` packages doivent la valeur SDK du projet `Microsoft.NET.Sdk.Web` en haut du fichier projet (`<Project Sdk="Microsoft.NET.Sdk.Web">`).
* Projets faisant référence à des packages ou des projets qui font référence à transitivement `Microsoft.AspNetCore.All` ou `Microsoft.AspNetCore.App`:
  * Doit définir le Kit de développement du projet `Microsoft.NET.Sdk.Web` en haut du fichier projet (`<Project Sdk="Microsoft.NET.Sdk.Web">`).
  * Doit référencer le même package d’exécution partagé. Si LibraryA fait référence à `Microsoft.AspNetCore.App`, tous les projets référençant LibraryA doivent également référencer `Microsoft.AspNetCore.App`.
* Projets exécutables (projets qui contiennent des applications qui sont lancées avec `dotnet run` ou applications qui s’exécutent et tester des projets pour les applications) ne doit pas spécifier une version pour `Microsoft.AspNetCore.App`. Le Kit de développement spécifie la version implicitement via `<PackageReference Include="Microsoft.AspNetCore.App" />`.
* Projets référencés (projets qui ne sont pas le point d’entrée et les références de projet `Microsoft.AspNetCore.All` ou `Microsoft.AspNetCore.App`) doit spécifier une version du package.
