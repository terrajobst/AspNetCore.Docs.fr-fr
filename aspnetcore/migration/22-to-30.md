...
* Dans la `<ItemGroup>`:
  * `Microsoft.AspNetCore.App` est supprimée. Pour plus d’informations, consultez Référence de l' [infrastructure](#framework-reference) dans ce document.
...
* Les projets qui ciblent `Microsoft.NET.Sdk` ou `Microsoft.NET.Sdk.Razor` Kit de développement logiciel (SDK) doivent ajouter un `FrameworkReference` explicite à `Microsoft.AspNetCore.App`:
...
ASP.NET Core 3,0 supprime certains assemblys qui faisaient auparavant partie de la référence du package `Microsoft.AspNetCore.App`. Pour visualiser les assemblys qui ont été supprimés, comparez les deux dossiers d’infrastructure partagés. Par exemple, une comparaison des versions 2.2.7 et 3.0.0 :
...
* L’authentification &ndash; la prise en charge des flux d’authentification tiers sont disponibles sous forme de packages NuGet :
...
* La prise en charge de MVC `Newtonsoft.Json` (Json.NET) &ndash; la prise en charge de l’utilisation de MVC avec `Newtonsoft.Json` fait désormais partie de [Microsoft. AspNetCore. Mvc. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
* `IHostingEnvironment` à `IWebHostEnvironment`, consultez [cette annonce GitHub](https://github.com/dotnet/AspNetCore/issues/7749).
...
Les projets qui ciblent `Microsoft.NET.Sdk.Web` des analyseurs de référence implicitement fournis précédemment dans le cadre du package [Microsoft. AspNetCore. Mvc. Analysis](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Analyzers/) . Aucune référence supplémentaire n’est requise pour les activer.
...
* les `Microsoft.AspNetCore.Server.Kestrel.Transport.Abstractions` et les types associés ont été supprimés.
* <xref:Microsoft.AspNetCore.Server.Kestrel.KestrelServerOptions.NoDelay> a été déplacée de <xref:Microsoft.AspNetCore.Server.Kestrel.Core.ListenOptions> vers les options de transport.
* `Microsoft.AspNetCore.Server.Kestrel.Transport.Abstractions.Internal.SchedulingMode` a été supprimée du <xref:Microsoft.AspNetCore.Server.Kestrel.KestrelServerOptions>.
...
Cela pose des problèmes d’ambiguïté entre les en-têtes et les codes de fin. par conséquent, les codes de fin ont été déplacés vers une nouvelle collection (`RequestTrailerExtensions`) dans 3,0.
...
Pour la version 3,0, les méthodes de `RequestTrailerExtensions` suivantes sont disponibles :

* `GetDeclaredTrailers` &ndash; obtient l’en-tête de `Trailer` de demande qui répertorie les codes de fin à attendre après le corps.
...
* `GetTrailer` &ndash; obtient l’en-tête de fin demandé à partir de la réponse. Vérifiez `SupportsTrailers` avant d’appeler `GetTrailer`, ou un <xref:System.NotSupportedException> peut se produire si la requête ne prend pas en charge les en-têtes de fin.
...
`AllowSynchronousIO` active ou désactive les API d’e/s synchrones, telles que `HttpRequest.Body.Read`, `HttpResponse.Body.Write`et `Stream.Flush`. Ces API sont une source de privation de thread conduisant à des blocages d’application. Dans 3,0, `AllowSynchronousIO` est désactivée par défaut. Pour plus d’informations, consultez [la section e/s synchrone dans l’article Kestrel](/aspnet/core/fundamentals/servers/kestrel?view=aspnetcore-3.0#synchronous-io).
...
Le sérialiseur JSON par défaut pour ASP.NET Core est désormais <xref:System.Text.Json>, qui est une nouveauté de .NET Core 3,0. Envisagez d’utiliser `System.Text.Json` lorsque cela est possible. Elle est très performante et ne nécessite pas de dépendance de bibliothèque supplémentaire. Toutefois, étant donné que `System.Text.Json` est nouveau, il est possible qu’il manque actuellement des fonctionnalités dont votre application a besoin. Pour plus d’informations, consultez [Comment migrer de Newtonsoft. JSON vers System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to).

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-opno-locsignalr-project"></a>Utiliser Newtonsoft. JSON dans un projet ASP.NET Core 3,0 SignalR
...
* Mettez à jour `Startup.ConfigureServices` pour appeler `AddNewtonsoftJson`.
  ...
  * `AddControllers`
  ...
  `Newtonsoft.Json` paramètres peuvent être définis dans l’appel à `AddNewtonsoftJson`:
...
Trois nouvelles méthodes d’extension de niveau supérieur liées aux scénarios MVC sur `IServiceCollection` sont disponibles. Les modèles utilisent ces nouvelles méthodes au lieu de `AddMvc`. Toutefois, `AddMvc` continue de se comporter comme dans les versions précédentes.
...
* Si l’application accède directement `RouteData.Routers` dans MVC pour analyser les URL, vous pouvez remplacer ceci par l’utilisation de [LinkParser. ParsePathByEndpointName](xref:Microsoft.AspNetCore.Routing.LinkParserEndpointNameAddressExtensions.ParsePathByEndpointName*). 
...
Pour la plupart des applications, seules les `Startup` requièrent des modifications.
...
Dans ASP.NET Core 3,0 ou version ultérieure, `UseAuthorization` est appelé dans `Startup.Configure`et le `HomeController` suivant nécessite un utilisateur connecté :
...
Lors de l’utilisation du routage de point de terminaison, nous vous recommandons de configurer `<xref:Microsoft.AspNetCore.Mvc.Authorization.AuthorizeFilter>` plutôt que d’utiliser l’intergiciel d’autorisation.  Si l’application utilise un `AuthorizeFilter` en tant que filtre global dans MVC, nous vous recommandons de refactoriser le code pour fournir une stratégie dans l’appel à `AddAuthorization`.

Le `DefaultPolicy` est initialement configuré pour exiger l’authentification, donc aucune configuration supplémentaire n’est requise. Dans l’exemple suivant, les points de terminaison MVC sont marqués comme `RequireAuthorization`, de sorte que toutes les demandes doivent être autorisées en fonction du `DefaultPolicy`. Toutefois, l' `HomeController` autorise l’accès sans que l’utilisateur se connecte à l’application en raison d' `[AllowAnonymous]`:
...
L’autorisation peut également être configurée pour des classes spécifiques de points de terminaison. Le code suivant est un exemple de conversion d’une application MVC qui a configuré un `AuthorizeFilter` global pour une application avec une stratégie spécifique nécessitant une autorisation :
...
En outre, chaque point de terminaison peut personnaliser ses exigences d’autorisation. Dans l’exemple suivant, `UseAuthorization` traite l’autorisation avec le `DefaultPolicy`, mais le point de terminaison de contrôle d’intégrité `/healthz` requiert un utilisateur `admin` :
...
Dans ASP.NET Core 2,2, vous pouviez définir la `TransportMaxBufferSize` et cela permettrait de contrôler la taille maximale des messages. Dans ASP.NET Core 3,0, cette option contrôle désormais uniquement la taille maximale avant que la contre-pression soit observée.
...
Dans ASP.NET Core 3,0, ASP.NET Core MVC supprime le suffixe `Async` des noms d’action du contrôleur. Le routage et la génération de liens sont affectés par cette nouvelle valeur par défaut. Par exemple :
...
* Génération de lien requise spécifiant le suffixe `Async`. Par exemple :
...
* La génération de liens ne nécessite pas la spécification du suffixe `Async`. Par exemple :
...
Comme expliqué dans la documentation sur les [différences par rapport aux versions antérieures du routage](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), il existe quelques différences dans la génération de liens (à l’aide de `Url.Link` et d’API similaires, par exemple). notamment :
...
Le mappage de Razor Pages a maintenant lieu dans `UseEndpoints`.
...
L’utilisation de MVC via `UseMvc` ou `UseMvcWithDefaultRoute` dans ASP.NET Core 3,0 requiert un abonnement explicite dans `Startup.ConfigureServices`. Cela est nécessaire, car MVC doit savoir s’il peut reposer sur l’autorisation et l’intergiciel CORS pendant l’initialisation. Un analyseur est fourni pour avertir si l’application tente d’utiliser une configuration non prise en charge.

Si l’application nécessite une prise en charge de `IRouter` héritée, désactivez `EnableEndpointRouting` à l’aide de l’une des approches suivantes dans `Startup.ConfigureServices`:
...
Ajoutez `MapHealthChecks` pour utiliser les contrôles d’intégrité avec routage des points de terminaison. La méthode `MapHealthChecks` accepte des arguments similaires à `UseHealthChecks`. L’avantage de l’utilisation de `MapHealthChecks` sur `UseHealthChecks` est la possibilité d’appliquer une autorisation et de bénéficier d’un contrôle plus précis de la stratégie de correspondance.

Dans l’exemple suivant, `MapHealthChecks` est appelé pour un point de terminaison de contrôle d’intégrité au `/healthz`:
...
Les modèles ASP.NET Core 3,0 utilisent l' [hôte générique](xref:fundamentals/host/generic-host). Les versions précédentes utilisaient l' [hôte Web](xref:fundamentals/host/web-host). Le code suivant montre le modèle ASP.NET Core 3,0 généré `Program` classe :
...
<xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder> reste dans 3,0 et est le type du `webBuilder` vu dans l’exemple de code précédent. les <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> seront dépréciées dans une version ultérieure et remplacées par `HostBuilder`.

La modification la plus importante de `WebHostBuilder` à `HostBuilder` est dans l' [injection de dépendances (di)](xref:fundamentals/dependency-injection). Lorsque vous utilisez `HostBuilder`, vous pouvez uniquement injecter les éléments suivants dans le constructeur de `Startup`:
...
* Basculer vers à l’aide de `AddAuthorizationCore`
...
`System.Text.Json` est désormais le protocole de concentrateur par défaut utilisé par le client et le serveur.
...
L’exemple de https://github.com/aspnet/samples/tree/master/samples/aspnetcore/mvc/runtimecompilation montre un exemple d’activation conditionnelle de la compilation au moment de l’exécution dans les environnements de développement.
...
Le système de validation dans .NET Core 3,0 et versions ultérieures traite les paramètres non Nullable ou les propriétés liées comme s’ils avaient un attribut `[Required]`. Pour plus d’informations, consultez [attribut [required]](xref:mvc/models/validation?view=aspnetcore-3.1#required-attribute).
