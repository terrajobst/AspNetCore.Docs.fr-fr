...
::: moniker-end
Une alternative à l’injection de `IWebHostEnvironment` consiste à utiliser une approche basée sur les conventions.
::: moniker range=">= aspnetcore-3.0"
...
Utilisez <xref:Microsoft.AspNetCore.Hosting.IStartupFilter> pour configurer le middleware au début ou à la fin du pipeline de middleware de la méthode [Configure](#the-configure-method) d’une application. Utilisez `IStartupFilter` pour créer un pipeline de méthodes `Configure`. [IStartupFilter.Configure](xref:Microsoft.AspNetCore.Hosting.IStartupFilter.Configure*) peut définir un middleware à exécuter avant ou après les middlewares qui sont ajoutés par les bibliothèques.
