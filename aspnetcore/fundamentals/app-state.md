...
ASP.NET Core conserve l’état de session en donnant au client un cookie qui contient l’ID de session envoyé au serveur avec chaque requête. Le serveur utilise ensuite cet ID de session pour récupérer (fetch) les données de session. Le cookie de session étant spécifique au navigateur, le partage de sessions entre navigateurs n’est pas possible. Les cookies de session sont supprimés seulement à la fin de la session du navigateur. Si un cookie est reçu pour une session expirée, une nouvelle session utilisant le même cookie de session est créée. 

Le serveur conserve une session pendant une durée limitée après la dernière requête. Vous pouvez définir le délai d’expiration de session souhaité ou utiliser la valeur par défaut de 20 minutes. L’état de session est idéal pour le stockage de données utilisateur qui sont spécifiques à une session particulière, mais qui ne nécessitent pas d’être conservées de façon permanente. Les données sont supprimées du magasin de stockage quand `Session.Clear` est appelé ou quand la session expire dans le magasin de données. Le serveur ne peut pas déterminer quand le navigateur est fermé ou quand le cookie de session est supprimé.
