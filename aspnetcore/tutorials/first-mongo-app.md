...
* [Visual Studio 2017 version 15.9 ou ultérieure](https://www.visualstudio.com/downloads/) avec la charge de travail **ASP.NET et développement web**
...
1. Dans la fenêtre **Console du Gestionnaire de Package**, accédez à la racine du projet. Exécutez la commande suivante afin d’installer le pilote .NET pour MongoDB :
...
1. Ouvrez **Terminal intégré** et accédez à la racine du projet. Exécutez la commande suivante afin d’installer le pilote .NET pour MongoDB :
...
Dans la classe précédente, la propriété `Id` est requise pour mapper l’objet Common Language Runtime (CLR) à la collection MongoDB. Les autres propriétés de la classe reçoivent l’attribut `[BsonElement]`. La valeur de l’attribut représente le nom de la propriété dans la collection MongoDB.
