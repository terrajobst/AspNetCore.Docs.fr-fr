...
Ce tutoriel vous montre comment créer une application avec Razor Components côté serveur ou avec Blazor côté client.

Pour utiliser ASP.NET Core Razor Components côté serveur (*recommandé*) :

* Suivez les instructions concernant l’*expérience Razor Components* dans <xref:blazor/get-started#server-side-razor-components-experience> pour créer un projet Razor Components.
* Attribuez un nom au projet `RazorComponents`.

Pour une expérience à l’aide de Blazor :

* Suivez les instructions concernant l’*expérience Blazor* dans <xref:blazor/get-started#client-side-blazor-experience> pour créer un projet Blazor.
* Attribuez un nom au projet `Blazor`.

[Affichez ou téléchargez un exemple de code](https://github.com/aspnet/Docs/tree/master/aspnetcore/tutorials/build-your-first-blazor-app/samples/) ([procédure de téléchargement](xref:index#how-to-download-a-sample)). Pour connaître les prérequis, consultez les rubriques suivantes :
