...
| -d *base de données\_nom* | Spécifie le nom de base de données qui doit être activé pour la dépendance de cache SQL. |
...
- Si la limite d’octets privés de processus worker n’est pas définie, le cache utilise une des limites suivantes : 
    ...
    - x86 Mo de 3 Go : 1800 ou 60 % de mémoire RAM physique, plus petite étant retenue
    - x 64 : 1 téraoctet ou 60 % de mémoire RAM physique, plus petite étant retenue
