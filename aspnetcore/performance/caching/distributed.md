...
La configuration du cache est spécifique à l’implémentation. Cet article décrit comment configurer les caches distribués Redis et SQL Server. IIndépendamment de l’implémentation sélectionnée, l’application interagit avec le cache à l’aide d’une interface `IDistributedCache` commune.
...
Dans l’exemple de code, une implémentation `Staging` est utilisée lorsque le serveur est configuré pour un environnement `RedisCache`. Par conséquent, la méthode `ConfigureStagingServices` configure le `RedisCache`:
