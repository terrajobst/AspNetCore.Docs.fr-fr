...
---
# <a name="aspnet-core-middleware-fundamentals"></a>Notions de base de l’intergiciel (middleware) ASP.NET Core
...
La rubrique [Migration des modules HTTP vers les intergiciels (middleware)](../migration/http-modules.md) explique la différence entre les pipelines de requête dans ASP.NET Core et dans les précédentes versions, puis fournit d’autres exemples d’intergiciel.
...
![Modèle de traitement des requêtes montrant une requête qui arrive et qui est traitée par trois intergiciels, puis la réponse qui quitte l’application. Chaque intergiciel exécute sa logique et transmet la requête à l’intergiciel suivant à l’instruction next(). Une fois que le troisième intergiciel a traité la requête, elle est repassée aux deux intergiciels précédents pour être à nouveau traitée après les instructions next(), l’une après l’autre, avant de quitter l’application sous forme de réponse au client.](middleware/_static/request-delegate-pipeline.png)
...
Le tableau suivant présente les requêtes et réponses de `http://localhost:1234` avec le code précédent :
