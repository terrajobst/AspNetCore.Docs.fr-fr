...
* Sélectionnez **ASP.NET Core 2.0** dans la liste déroulante, puis sélectionnez **Application web**.
 ![Application web (pages Razor)](../../razor-pages/index/_static/np2.png)
...
La propriété `ID` devient la colonne de clé primaire de la table de base de données qui correspond à cette classe. Par défaut, EF Core interprète une propriété nommée `ID` ou `classnameID` comme clé primaire. Dans `classnameID`, `classname` est le nom de la classe, comme `Student` dans l’exemple précédent.
...
La chaîne de connexion précédente utilise `ConnectRetryCount=0` pour empêcher le blocage de [SQLClient](/dotnet/framework/data/adonet/ef/sqlclient-for-the-entity-framework).
...
* Le mot clé `async` fait en sorte que le compilateur :
  ...
  * Crée automatiquement l’objet [Task](/dotnet/api/system.threading.tasks.task?view=netframework-4.7) qui est retourné. Pour plus d’informations, consultez [Type de retour Task](/dotnet/csharp/programming-guide/concepts/async/async-return-types#BKMK_TaskReturnType).
...
Pour plus d’informations sur les méthodes asynchrones dans .NET, consultez [Vue d’ensemble d’async](/dotnet/articles/standard/async).
