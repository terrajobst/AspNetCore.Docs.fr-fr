...
Dans ce document, un *chemin d’accès du code à chaud* est défini comme un chemin d’accès de code qui est fréquemment appelé et où une grande partie de la durée d’exécution se produit. Chemins d’accès du code à chaud limitent généralement l’application scale-out et performances.
...
* Rendre le contrôleur/Razor actions sur la Page asynchrone. La pile d’appel entière est asynchrone pour tirer parti de [async/await](/dotnet/csharp/programming-guide/concepts/async/) modèles.

Un profileur, tel que [PerfView](https://github.com/Microsoft/perfview), peut être utilisé pour rechercher des threads fréquemment ajoutés à la [du Pool de threads](/windows/desktop/procthread/thread-pools). Le `Microsoft-Windows-DotNETRuntime/ThreadPoolWorkerThread/Start` événement indique un thread ajouté au pool de threads. <!--  For more information, see [async guidance docs](TBD-Link_To_Davifowl_Doc  -->
...
Problèmes de mémoire, telles que l’exemple précédent, peuvent être diagnostiqués en examinant les statistiques des garbage collection (GC) dans [PerfView](https://github.com/Microsoft/perfview) et examen :
...
Interactions avec un magasin de données et d’autres services à distance sont souvent les parties les plus lentes d’une application ASP.NET Core. Lire et écrire les données efficacement sont essentiel pour de bonnes performances.
...
* **Faire** envisager la mise en cache fréquemment accédé à des données récupérées à partir d’une base de données ou d’un service distant, si les données périmées légèrement sont acceptables. Selon le scénario, vous devez utiliser un [MemoryCache](xref:performance/caching/memory) ou un [DistributedCache](xref:performance/caching/distributed). Pour plus d'informations, consultez <xref:performance/caching/response>.
* **Faire** réduire les allers-retours réseau. L’objectif est de récupérer les données requises dans un seul appel plutôt que plusieurs appels.
...
* **Faire** envisager que EF Core résout certains opérateurs de requête sur le client, ce qui peut entraîner l’exécution de requête inefficace. Pour plus d’informations, consultez [des problèmes de performances d’évaluation Client](/ef/core/querying/client-eval#client-evaluation-performance-issues).
...
Nous vous recommandons de mesurer l’impact des approches hautes performances précédents avant de valider la base de code. La complexité supplémentaire des requêtes compilées ne justifie pas l’amélioration des performances.
...
Vous voulez tout votre code pour être rapide, souvent appelée chemins de code sont les plus importantes à optimiser :
...
* **Faire** utiliser les outils de profilage, telles que des performances [les outils de Diagnostic de Visual Studio](/visualstudio/profiling/profiling-feature-tour) ou [PerfView](https://github.com/Microsoft/perfview)), pour identifier [à chaud des chemins de code](#hot).
...
* **Faire** prendre en compte la gestion des requêtes longues avec [services d’arrière-plan](xref:fundamentals/host/hosted-services) ou en dehors du processus avec un [Azure Function](/azure/azure-functions/). Fin de travail out-of-process est particulièrement utile pour les tâches sollicitant beaucoup le processeur.
...
* Minimisation, ce qui réduit la taille des fichiers en supprimant les espaces blancs et des commentaires.
...
Chaque nouvelle version d’ASP.NET Core inclut des améliorations de performances. Optimisations dans .NET Core et ASP.NET Core signifient que les versions plus récentes généralement plus performantes que les versions antérieures. Par exemple, .NET Core 2.1 prise en charge pour les expressions régulières compilées et bénéficié de [ `Span<T>` ](https://msdn.microsoft.com/magazine/mt814808.aspx). ASP.NET Core 2.2 ajouté prise en charge HTTP/2. Si les performances sont une priorité, envisagez la mise à niveau vers la version actuelle d’ASP.NET Core.
...
* **Ne le faites pas** utilisez lever ou intercepter des exceptions comme un moyen de flux normal du programme, en particulier dans les [à chaud des chemins de code](#hot).
...
Les outils de diagnostic application, tels que Application Insights, peuvent aider à identifier les exceptions courantes dans une application qui peut affecter les performances.
