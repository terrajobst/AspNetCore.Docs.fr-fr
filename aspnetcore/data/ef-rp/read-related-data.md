...
---
# <a name="reading-related-data---ef-core-with-razor-pages-6-of-8"></a>Lecture des données associées - Pages Razor avec EF Core (6 sur 8)
...
Dans ce didacticiel, les données associées sont lues et affichées. Il s'agit des données qu'EF Core charge dans les propriétés de navigation.

Si vous rencontrez des problèmes que vous n'arrivez pas à résoudre, téléchargez l'[application terminée pour cette étape](https://github.com/aspnet/Docs/tree/master/aspnetcore/data/ef-rp/intro/samples/StageSnapShots/cu-part6-related).
...
Le chargement hâtif envoie plusieurs requêtes lorsqu’une collection de navigation est incluse :

 * Une requête pour la requête principale 
* Une requête pour chaque collection "en marge" (edge) dans l’arborescence de chargement.
...
Les structures de commande précèdent le modèle `Course`. Ouvrez le projet dans Visual Studio.
...
`1>Pages/Courses/Index.cshtml.cs(26,37,26,43): error CS1061: 'SchoolContext' does not
 ...
 Remplacez globalement `_context.Course` par `_context.Courses` (autrement dit, ajoutez un « s » à `Course`). 7 occurrences trouvées et mises à jour.
...
Exécutez l’application et sélectionnez le lien **Courses**. La colonne Service affiche la propriété `DepartmentID`, ce qui n’est pas utile.
...
Mettez à jour *Views/Courses/Index.cshtml* avec le balisage en surbrillance suivant :

[!code-html[](intro/samples/cu/Pages/Courses/Index.cshtml?highlight=4,7,15-17,34-36,44)]

Les modifications suivantes ont été apportées au code de modèle généré automatiquement :

* Remplacement du titre Index par Courses.
* Ajoutez une colonne **Number** qui affiche la valeur de la propriété `CourseID`. Par défaut, les clés primaires ne sont pas générées, car elles n'ont normalement pas de sens pour les utilisateurs finaux. Dans ce cas cependant, la clé primaire est significative.
* Modification de la colonne **Department** pour afficher le nom du service. Le code affiche la propriété`Name` de l'entité `Department` qui est chargée dans la propriété de navigation `Department` :
...
<a name="select"></a>
### <a name="loading-related-data-with-select"></a>Chargement des données associées à Select

La méthode `OnGetAsync` charge les données associées à la méthode `Include` :

[!code-csharp[Main](intro/samples/cu/Pages/Courses/Index.cshtml.cs?name=snippet_RevisedIndexMethod&highlight=4)]

	L'opérateur `Select` charge uniquement les données connexes nécessaires. Pour les éléments uniques, comme `Department.Name` il utilise une jointure interne SQL. Pour les collections, il utilise un autre accès en base de données, tout comme l'opérateur `Include` sur les collections.
...
## <a name="create-an-instructors-page-that-shows-courses-and-enrollments"></a>Créer une page Instructeurs qui affiche les Cours et les Inscriptions
...
* Lorsque l’utilisateur sélectionne un cours (chimie dans l’image précédente), les données associées à partir de l'entité `Enrollments` sont affichées. Dans l’image précédente, le nom et le niveau de l'étudiant s’affichent. `Course` et `Enrollment` sont des entités dans une relation un-à-plusieurs.
...
La commande précédente structure le modèle `Instructor`. Ouvrez le projet dans Visual Studio

Générez le projet. La build génère des erreurs.

Modifiez globalement `_context.Instructor` en `_context.Instructors` (autrement dit, ajoutez un « s » à `Instructor`). 7 occurrences sont trouvées et mises à jour.
...
La requête comporte deux inclusions :
...
Exécutez l’application et sélectionnez l'onglet **Instructors**. La page affiche la `Location` (bureau) de l'entité `OfficeAssignment` associée. Si 'OfficeAssignment' est null, une cellule de tableau vide s’affiche.
...
La méthode `Where` retourne une collection. Dans la précédente méthode `Where`, un seul `Instructor` est retourné. La méthode `Single` convertit la collection en une seule entité `Instructor`. L'entité `Instructor` fournit l’accès à la propriété `CourseAssignments`. `CourseAssignments` fournit l’accès aux entités `Course` associées.
...
La balise précédente affiche une liste des cours associés à un instructeur lorsqu’un instructeur est sélectionné.

Testez l’application. Cliquez sur un lien **Select** dans la page des instructeurs.
...
Mettez à jour la requête de la méthode `OnGetAsync` dans *Pages/Instructors/Index.cshtml.cs* avec le code suivant :
...
L’approche `Single` précédente ne présente aucun avantage par rapport à `Where`. Certains développeurs préfèrent le style d'approche `Single`.
...
Supposons que les utilisateurs ne souhaitent pas particulièrement voir les inscriptions d'un cours. Dans ce cas, une optimisation consisterait à charger uniquement les données d’inscription si elles sont demandées. Dans cette section, la méthode `OnGetAsync` est mise à jour pour utiliser le chargement explicite de `Enrollments` et `Students`.

Mettez à jour la méthode `OnGetAsync` à l'aide du code suivant :
...
Notez que le code précédent commente `.AsNoTracking()`. Les propriétés de navigation peuvent uniquement être chargées explicitement pour les entités suivies.
