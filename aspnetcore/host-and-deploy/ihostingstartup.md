...
Une implémentation de [IHostingStartup](/dotnet/api/microsoft.aspnetcore.hosting.ihostingstartup) permet d’ajouter des fonctionnalités à une application au démarrage à partir de l’application en dehors de la classe `Startup`. Par exemple, une bibliothèque d’outils externes permettre utiliser une implémentation de `IHostingStartup` pour fournir des services à une application ou des fournisseurs de configuration supplémentaires. `IHostingStartup` *est disponible dans ASP.NET Core 2.0 et les versions ultérieures.*
