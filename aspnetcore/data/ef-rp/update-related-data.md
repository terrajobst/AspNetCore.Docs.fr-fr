...
Examinez et testez les pages de création et de modification de cours. Créez un nouveau cours. Le département est sélectionné par sa clé primaire (entier), pas son nom. Modifiez le nouveau cours. Lorsque vous avez terminé le test, supprimez le nouveau cours.

## <a name="create-a-base-class-to-share-common-code"></a>Créer une classe de base pour partager le code commun

Les pages Course/Create et Course/Edit ont besoin d’une liste de noms de département. Créez la classe de base*Pages/Courses/DepartmentNamePageModel.cshtml.cs* pour les pages Create et Edit :
...
Les classes de modèle de page Create et Edit doit dériver de `DepartmentNamePageModel`.

## <a name="customize-the-courses-pages"></a>Personnaliser les Pages de cours

Lorsqu’une nouvelle entité de cours est créée, il doit avoir une relation à un département existant. Pour ajouter un département lors de la création d’un cours, la classe de base pour Create et Edit contient une liste déroulante pour sélectionner le département. La liste déroulante définit la propriété de clé étrangère (FK) `Course.DepartmentID`. EF Core utilise la FK `Course.DepartmentID` pour charger la propriété de navigation `Department`.
...
`ViewData["DepartmentID"]`est remplacé par fortement typé `DepartmentNameSL`. Les modèles fortement typés sont préférés à ceux faiblement typés. Pour plus d’informations, consultez [données faiblement typées (ViewData et ViewBag)](xref:mvc/views/overview#VD_VB).

### <a name="update-the-courses-create-page"></a>Mise à jour de la page de création de cours

Mettez à jour *Pages/Courses/Create.cshtml* par le balisage suivant :
...
* Ajoute l’option « Select department ». Cette modification affiche « Select départment » plutôt que le premier département.
...
Testeé la page de création. La page Create affiche le nom du département plutôt que l’ID de département.
...
Mettez à jour *Pages/Courses/Edit.cshtml* par le balisage suivant :
...
La page contient un champ masqué (`<input type="hidden">`) pour le numéro de cours. Ajout d’un tag helper `<label>` avec `asp-for="Course.CourseID"` n’enlève pas la nécessité du champ masqué. `<input type="hidden">` est requis pour le numéro de cours à inclure dans les données publiées lorsque l’utilisateur clique sur **Save**.
...
Mettez à jour la méthode `OnGetAsync` dans le *Pages/Courses/Details.cshtml.cs* fichier :

[!code-csharp[Main](intro/samples/cu/Pages/Courses/Details.cshtml.cs?name=snippet)]

### <a name="modify-the-delete-and-details-pages"></a>Modifier les pages Delete et détails

Mettez à jour de la page Supprimer un Razor avec le balisage suivant :

[!code-cshtml[Main](intro/samples/cu/Pages/Courses/Delete.cshtml?highlight=15-20)]

Apportez les mêmes modifications à la page de détails.

### <a name="test-the-course-pages"></a>Tester les pages de cours

Testez créer, modifier, détails et supprimer.
...
Lorsque vous modifiez un enregistrement d'instructeur, vous pouvez mettre à jour l’attribution du bureau. L'entité `Instructor` a une relation un-à-zéro-ou-un avec l'entité `OfficeAssignment`. Le code de formateur doit gérer :
...
* Si l’utilisateur modifie l’affectation d’un bureau, mettre à jour l'entité `OfficeAssignment`.

Mettre à jour le modèle de page Edit d'instructeur avec le code suivant :
...
### <a name="update-the-instructor-edit-page"></a>Mis à jour de la page Modifier le formateur

Mettez à jour de *Pages/Instructors/Edit.cshtml* avec l’emplacement du bureau :
...
`Course`et `Instructor` ont une relation plusieurs-à-plusieurs. Pour ajouter et supprimer des relations, vous ajoutez et supprimez des entités à partir de l'entité set joint `CourseAssignments`.

Des cases à cocher permmettent les modifications aux cours auxquels un formateur est assigné. Une case à cocher s’affiche pour chaque cours dans la base de données. Les cours auxquels appartient le formateur sont vérifiées. L’utilisateur peut sélectionner ou désactivez les cases à cocher pour modifier les affectations de cours. Si le nombre de cours étaient nettement supérieur :
...
Créer la classe de base *Pages/Instructors/InstructorCoursesPageModel.cshtml.cs* :

[!code-csharp[Main](intro/samples/cu/Pages/Instructors/InstructorCoursesPageModel.cshtml.cs)]

Le `InstructorCoursesPageModel` est la classe de base utilisée pour la modifier et créer des modèles de page. `PopulateAssignedCourseData`lit toutes les entités `Course` pour remplir `AssignedCourseDataList`. Pour chaque cours, le code définit la `CourseID`, le titre et s’il faut ou non le formateur est affecté au cours. Un [HashSet](https://docs.microsoft.com/dotnet/api/system.collections.generic.hashset-1) est utilisé pour créer des recherches efficaces.

### <a name="instructors-edit-page-model"></a>Modifier le modèle de page instructeurs

Mettez à jour le modèle de page de modification de formateur par le code suivant :
...
<a id="notepad"></a>
> [!NOTE]
> Lorsque vous collez le code dans Visual Studio, les sauts de ligne sont modifiées d’une manière qui casse le code. Appuyez une fois sur Ctrl + Z pour annuler la mise en forme automatique. CTRL + Z résout les sauts de ligne afin qu’ils apparaissent comme vous le voyez ici. La mise en retrait ne doit pas nécessairement être parfaite, mais les lignes `@</tr><tr>`, `@:<td>`, `@:</td>`, et `@:</tr>` doivent chacune être sur une seule ligne comme indiqué. Avec le nouveau bloc de code sélectionné, appuyez sur Tab trois fois pour aligner le nouveau code avec le code existant. Voter pour ou vérifier l’état de ce bogue sur [ce lien](https://developercommunity.visualstudio.com/content/problem/147795/razor-editor-malforms-pasted-markup-and-creates-in.html).

Le code précédent crée une table HTML qui possède trois colonnes. Chaque colonne possède une case à cocher et une légende contenant le numéro et le titre. Toutes les cases à cocher ont le même nom (« selectedCourses »). Utiliser le même nom informe le model binder de les traiter en tant que groupe. L’attribut value de chaque case à cocher a la valeur `CourseID`. Lorsque la page est publiée, le cmodel binder passe un tableau comprenant les valeurs `CourseID` pour les cases à cocher sélectionnées.

Lorsque les cases à cocher sont restitués initialement, les cours affectés à l'instrucetur ont l'attribut checked.
...
Remarque : L’approche adoptée ici pour modifier les données de cours formateur fonctionne bien quand un nombre limité de cours. Pour les collections qui sont beaucoup plus volumineuses, une interface utilisateur différente et une autre méthode de mise à jour serait plus utile et plus efficace.

### <a name="update-the-instructors-create-page"></a>Mise à jour de la page de création des instructeurs

Mettez à jour le modèle de page de création d'instructeur par le code suivant :

[!code-csharp[Main](intro/samples/cu/Pages/Instructors/Create.cshtml.cs)]

Le code précédent est similaire au code de *Pages/Instructors/Edit.cshtml.cs*.

Mettez à jour la page de création Razor d'un instructeur par le balisage suivant :

[!code-cshtml[Main](intro/samples/cu/Pages/Instructors/Create.cshtml?highlight=32-62)]

Tester la page de création d'un instructeur.

## <a name="update-the-delete-page"></a>Mise à jour de la page de suppression

Mettez à jour le modèle de page de suppression par le code suivant :
...
* Utilise un chargement hâtif pour la propriété de navigation `CourseAssignments`. Les `CourseAssignments` doivent être inclus ou ils ne seront pas supprimés lorsque le formateur est supprimé. Pour éviter d’avoir à les lire, configurez la suppression en cascade dans la base de données.
