...
* La prise en charge de MVC `Newtonsoft.Json` &ndash; la prise en charge de l’utilisation de MVC avec `Newtonsoft.Json` fait désormais partie de [Microsoft. AspNetCore. Mvc. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
La valeur par défaut pour ASP.NET Core est désormais [System. Text. JSON](/dotnet/api/system.text.json?view=netcore-3.0), qui est une nouveauté de .net Core 3,0. Envisagez d’utiliser `System.Text.Json` lorsque cela est possible. Elle est très performante et ne nécessite pas de dépendance de bibliothèque supplémentaire. Toutefois, étant donné que `System.Text.Json` est nouveau, il est possible qu’il manque actuellement des fonctionnalités dont votre application a besoin.
...
### <a name="systemruntimeserialization-attributes-not-supported"></a>Attributs System. Runtime. Serialization non pris en charge

Les attributs [[DataContract]](xref:System.Runtime.Serialization.DataContractAttribute) et [[DataMember]](xref:System.Runtime.Serialization.DataMemberAttribute) ne sont pas pris en charge dans `System.Text.Json`. Pour plus d’informations, consultez la [prise en charge de System. Text. Json par GitHub dans System. Runtime. Serialization](https://github.com/dotnet/corefx/issues/38758).
