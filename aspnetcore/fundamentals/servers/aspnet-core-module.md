...
La configuration pour le module ASP.NET Core est stockée dans le fichier *web.config* qui se trouve dans le dossier racine de l’application. Les paramètres de ce fichier pointent vers la commande de démarrage et les arguments qui démarrent votre application ASP.NET Core. Pour un exemple de code *web.config* et des conseils sur les options de configuration, consultez [référence de Configuration du module ASP.NET Core](xref:host-and-deploy/aspnet-core-module).
