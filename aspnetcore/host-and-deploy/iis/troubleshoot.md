...
* Azure App Service utilise également le [Module ASP.NET Core](xref:host-and-deploy/aspnet-core-module) et IIS pour héberger des applications. Pour des conseils de résolution des problèmes qui se rapportent spécifiquement à Azure App Service, consultez <xref:host-and-deploy/azure-apps/troubleshoot>.
* <xref:fundamentals/error-handling> Couvre comment gérer les erreurs dans des applications ASP.NET Core pendant le développement sur un système local.
* [Apprendre à déboguer avec Visual Studio](/visualstudio/debugger/getting-started-with-the-debugger) présente les fonctionnalités du débogueur Visual Studio.
* [Débogage avec Visual Studio Code](https://code.visualstudio.com/docs/editor/debugging) décrit la prise en charge du débogage intégré à Visual Studio Code.
